...
| **Discovery Successful** | Green check when both Port Check and Auth succeed. If either fails, this is marked as a failure (red X) only when the device is not added. Otherwise, based on the scores, it is marked as PARTIAL or OK. |
| **Sudo Access** | Whether sudo access is allowed. Green check if allowed, red cross if not, dash if not applicable. |
| **Ignored** | A [Device Ignore Rule](infrastructure-management/devices/device-ignore-rules.mdx) was applied and the device was excluded. |
