...
- Hover over elements in a chart or graphic to the breakdown to see values.
...
On the Insights+ homepage, there is an Import button in the top-right hand corner. It opens a pop-up that expects a `.zip` file which defines a chart or dashboard. 
...
- Alert content can also be either a dashboard or chart, but an alert is based on a condition you define for the alert using an SQL query that runs against a Device42 database (`d42_viewer_mt`). The condition check is triggered according to the schedule you set up for the alert
...
- **SQL Query**: Expects a SQL statement to poll Device42 for an aggregate value based on the input SQL. This means you will want to use a SQL statement that has a `WHERE` clause and provides the output as a single aggregate value like `COUNT`, `SUM`, `MAX`, `MIN`, and so on
...
Choose the **Time-series Smooth Line** to return a clean looking graph. In the _Time_ column, use **Start Time** from the dataset. **End Time** is also a suitable option depending on how you want to perform the analysis. Since it only returns day-level values, keep the time-grain as Day.

![](/assets/images/Insights_time-series-NM.png)

We want to measure the max Power Apparent from all PDUs, but let’s target the display by grouping the data on Room and Rack – so we can break down the totals into meaningful categories.
