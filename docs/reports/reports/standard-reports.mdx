...
:::note
Cloning Standard Reports is available only on on-premises installations.
:::
...
### Control Report Updates with Live Preview

Turn off the **Live Preview** toggle on the right to configure multiple filters without refreshing the report after each change. Turn it back on to refresh the results once using all your filters.

Device42 saves the selected default with the report definition and restores it when you reopen the report.

<ThemedImage
  alt="Standard Report results with the Live Preview toggle"
  sources={{
    light: useBaseUrl('/assets/images/standard-reports/standard-report-live-preview-light.png'),
    dark: useBaseUrl('/assets/images/standard-reports/standard-report-live-preview-dark.png'),
  }}
/>

### Filter Report Results

Click the filter icon in a column header to open its filter panel. Select an operator, enter a value, and click **Apply**.

To add more than one rule for the same column, click **Add Rule** and select how Device42 should combine the rules:

- **Match All** applies the filter when every rule matches (AND).
- **Match Any** applies the filter when at least one rule matches (OR).

You can apply filters to additional columns and use global search together with column filters.

![Advanced report filters using Match Any with multiple rules](/assets/images/standard-reports/advanced-filter-match-any.png)

For date columns, select **Date is**, **Date is before**, **Date is after**, or **Between**. Select **Between**, enter the start and end dates, and click **Apply** to show records within that date range.

![Between date filter on Standard Report results](/assets/images/standard-reports/advanced-filter-date-between.png)

Click **Clear Filters** to remove all active filters. Filters remain applied when you sort columns, move between result pages, or export the report. Filters match against the full set of report results, and the record count shows the total number of matching rows.

The following additional actions are available from the **Display on Screen** view:

- **Export:** Click **Export**, select the fields to include, and choose tab-separated values (TSV) or Excel format. The export applies the active data-grid filters and includes every matching row across all result pages, with no row limit. If **Live Preview** is off, Device42 runs the report with the current filters before creating the export.
- **Edit:** Click **Edit** to open the report details page directly in edit mode.
