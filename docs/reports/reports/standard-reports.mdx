...
- **Export:** Select the fields to include and export the report in tab-separated values (TSV) or Excel format. The export applies the active data-grid filters and includes every matching row, with no row limit. If **Live Preview** is off, Device42 runs the report with the current filters before creating the export.
