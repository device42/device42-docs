...
InsightsAI generates queries from the natural language descriptions you provide to quickly generate DOQL queries and tailored reports from your CMDB. The prompts you enter are sent to the `ai42.device42.io` endpoint.

When you type plain English messages, like “Which SSL/TLS certificates are expiring in the next 90 days?” or "Find all Resources where name contains 'Nested'," InsightsAI creates queries that can be executed without requiring deep knowledge of the query language or data model. You can preview the results and refine the query as needed.

<ThemedImage
  alt="InsightsAI menu location"
  sources={{
    light: useBaseUrl('/assets/images/insightsai/insights-chat-light.png'),
    dark: useBaseUrl('/assets/images/insightsai/insights-chat-dark.png'),
  }}
/>
...
<ThemedImage
  alt="InsightsAI UI"
  sources={{
    light: useBaseUrl('/assets/images/insightsai/insightsai-ui-light.png'),
    dark: useBaseUrl('/assets/images/insightsai/insightsai-ui-dark.png'),
  }}
/>

The two buttons above the input box get you started with an initial query that you can refine by submitting follow-up messages. Click **Device report** to start a new chart with the input:
...
InsightsAI generates an SQL query with the table and parameters in response to your input. See [Database Viewer Schema](reports/device42-doql/db-viewer-schema.mdx) for information on locating database field names and descriptions.
...
If the data or query needs to be refined, ask a follow-up question, and InsightsAI will provide a new response.  

For example, the previous results did not include information on device operating systems. You can give InsightsAI another direction to include operating systems and find devices running Windows.
...
- **(1) Export:** Download a `.tsv` or `.xls` file of the query result. 
- **(2) Save and Display On Screen:**  View, search, and print the data records.
...
Click **More Filters** to filter the chat queries by specific initial queries, users, and creation dates.
...
Delete one or more chats by selecting them and choosing a delete option from the **Actions** dropdown.
