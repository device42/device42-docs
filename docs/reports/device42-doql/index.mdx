...
:::tip
Use the [InsightsAI chat](reports/device42-doql/insightsai.mdx), under **Analytics > InsightsAI** on the MA, to create and interpret DOQL query snippets quickly in the context of the Device42 data tables.
:::
...
See the [Database Viewer Schema page](reports/device42-doql/db-viewer-schema.mdx) for information on the DB viewer schema and Entity Relationship Diagram, both of which are very handy when authoring queries, providing valuable insight into the Device42 database schema.
...
- Click **Create** to add a new query from the DOQL Query View list page.
...
- You can now make any necessary edits to the DOQL query you were looking at. Click the **Save** button when you are done editing your DOQL Query.
...
Generate DOQL queries by entering natural language descriptions in the **InsightsAI** chat, under **Analytics > InsightsAI** on the MA. See the [InsightsAI page](reports/device42-doql/insightsai.mdx) for more details.

You can also refer to the Entity Relationship Diagram and Data Dictionary table to get the names of Device42 database tables and columns (fields), and look up relationships. See the [Database Viewer Schema page](reports/device42-doql/db-viewer-schema.mdx) for more details.
