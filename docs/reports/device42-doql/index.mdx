...
Generate DOQL queries by entering natural language descriptions in the **InsightsAI** chat under **Analytics > InsightsAI** of the MA. See [InsightsAI page](insightsai.mdx) for details.

You can also refer to the Entity Relationship Diagram schematics and Data Dictionary table to get the names of Device42 database tables and columns (fields) and understand the database schema. See the [Database Viewer Schema page](db-viewer-schema.mdx) for more details.

Please note that Device42 does not provide assistance with writing DOQL queries for you. However, if you encounter a bug, please report it to our support team at [support@device42.com](mailto:support@device42.com) and we will fix it as part of our normal bug-fixing process.
