...
Updating to the latest Device42 ensures you get the most out of your CMDB and benefit from the newest features, integrations, security, and performance improvements.
...
-  From the Main Appliance, navigate to **Tools > Update** and click the [**Check for latest version** link](https://www.device42.com/update/). 
    ...
    You can visit [https://www.device42.com/update/](https://www.device42.com/update/) directly, however, it's useful to navigate to the link from the **Update Software** page, as you can take note of your current version to later verify a successful update.
...
- You'll receive an automated email with several links to download the `.zip.enc` file. Choose the first link (in bold) if your Main Appliance has already been updated to the latest [major](https://semver.org/) version and you're applying a minor or patch release update, or if the latest update is to a major version. 
...
:::note
The default credentials for the Appliance Manager are `d42admin / default` or for AWS instances `d42admin / AWS_INSTANCE_ID`. Please ensure you change these to your own secure credentials.
:::
...
There are two ways to log into the VM console:

1. **From VM Console:** Access the VM console through your virtual platform.
2. **Via SSH:** 
   - Use port 404 on the Device42 IP address.
   - If you're using a power appliance, use port 442.

For AWS deployments, SSH into the device using the username `device42` and password: `AWS-KEYFILE`. For example,
...
Select option **a** in the console to apply the update. It usually takes between 15 to 30 minutes to apply the update. 
...
## Updating the Remote Collector

When the Device42 main appliance is updated, the updates are automatically applied to your Remote Collectors as part of the overall Device42 update process. This happens as long as the Remote Collectors are connected to a Device42 Main Appliance.
