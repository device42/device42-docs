...
## Update the Device42 Main Appliance
...
:::tip
Watch the quick [How To Update the Device42 Main Appliance](/how-to-videos/update-device42-appliance-how-to) walkthrough video of the process.
:::

### 1. Obtain the Update File or URL
...
- You will receive an automated email with several links to download the `.zip.enc` file. If your Main Appliance has already been updated to the latest [major](https://semver.org/) version and you're applying a minor or patch-release update, or if your MA's latest update was to a major version, choose the first link (in bold).
...
For an explanation of the backup options, see the [Backup via the Appliance Manager](/administration/appliance-manager/setting-up-backup-device42-appliance-manager) page.
...
From the Main Appliance, navigate to **Application > Update Software** and **either** browse and upload the `.zip.enc` file **or** enter the update URL.
...
## Migrate Existing Power Appliance Jobs to a Remote Collector

You can use existing tools to migrate jobs that were created for the original, standalone power appliance to RCs.

Follow the [Power Job Migration Guide](/infrastructure-management/power-and-environmental-monitoring/power-rc-setup-job-migration) to migrate jobs to your choice of RCs.
