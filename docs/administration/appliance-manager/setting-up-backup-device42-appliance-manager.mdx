...
- **Backup TLS key pair** (for SFTP backups): Backup the key that was used to set up the SFTP server.
- **Backup Audit Data**: Backup the audit logs table, if disabled Device42 will backup the DB without that table.
- **Backup System Configuration Files**: Backup these system configuration files to preserve any manual config optimizations: `/data/pgsql/data/postgresql.override.conf` and `/etc/systemctl.conf`.

Ignore the "Tenant ID" field; it's a leftover from the old multi-tenancy project and not used anymore.
