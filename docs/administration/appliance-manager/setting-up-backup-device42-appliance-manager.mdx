...
The standby instance will share system identifiers with the production instance and will connect as the primary. For this reason, only one instance with the identifier data should be active at a time. See the [Warm HA Setup documentation](/administration/appliance-manager/warm-ha-setup-failover-and-automated-backups) for failover procedures.
...
If you intend to use a backup schedule for auto-restore to a standby appliance, as described in the [Warm HA Configuration](/administration/appliance-manager/warm-ha-setup-failover-and-automated-backups) documentation, you must select **Backup Meta Data** for the backup schedule.
...
See [Device42 Restore](/administration/appliance-manager/device42-restore).
