...
Device42 includes system-defined Auto Clean rules that you can use as provided or as the basis for your own rules. Some rules are enabled by default, while others are disabled examples that you can review and enable as needed.
...
### Remove Undetected Software Installations

Device42 includes the enabled **Remove Undetected Software Installations** rule. It deletes software-in-use records that have not been discovered or updated for more than 31 days relative to when their devices were last discovered or updated.

The rule has the following configuration:

- **Object Type:** Software In Use
- **Search Criteria:** Days Undetected is greater than `31`
- **Action:** Delete the data

You can edit or clone the rule to use a different threshold or other available criteria. If a matching software-in-use record cannot be deleted because it is assigned to a job, Device42 skips that record and continues running the rest of the Auto Clean job.

<ThemedImage
  alt="Remove Undetected Software Installations Auto Clean rule"
  sources={{
    light: useBaseUrl('/assets/images/auto-clean-feature/remove-undetected-software-installations-light.png'),
    dark: useBaseUrl('/assets/images/auto-clean-feature/remove-undetected-software-installations-dark.png'),
  }}
/>
