...
- A custom field marked as **Mandatory** must have a value in order to save the device (or other object) record as shown below. Please note that the field is only enforced as mandatory applies when objects are edited or added via the UI. The field is not mandatory when objects are edited or added via the API.
