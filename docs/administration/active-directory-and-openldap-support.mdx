...
- For the **Active Directory** type, choosing the **Domain\Username** username login style reveals the **Get Referral NetBIOS** options, which returns usernames in the format `ReferralNetbios/Username`.
...
- If you have selected the **Domain\Username** username login style, then users will be required to enter both the domain and username to log in into Device42
...
Test a user account for permission to query the AD.

- Save your settings to display the settings details page, and under the **ellipsis** (...) menu select **Test Connection**. 
...
- You will be prompted for credentials to initiate the test. Leave the credentials blank to use the currently selected username and password.
...
## Manually a Add Single User from Active Directory

To add a single new administrator user manually from Active Directory, navigate to **Tools > Admins & Permissions > Administrators** and click **Create Active Directory Admin**. 
...
You can review the domain found for the user found prior to adding them. The new user will be added according to the currently configured **Username login Style** option you selected when adding the LDAP settings. 
