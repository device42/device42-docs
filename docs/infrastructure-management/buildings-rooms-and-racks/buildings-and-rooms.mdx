...
Users with the [necessary permissions](/administration/role-based-access-control/) can access the [Room Layout View](#room-layout-view), to see how the room layout represents a room and its objects.

## Buildings List Page

View, edit, **Create** new buildings from the Buildings list page, as well as perform bulk and individual **[Actions](#buildings-and-rooms-actions)** on selected buildings. 

Filter the list by **Tags**, **Groups**, or add **More Filters** using the dropdown menus above the grid to narrow down the list.

You can add an image, like a floor plan or photo, to the building and view the image from the list page.
...
Find the **Rooms** list view under **Infrastructure > DataCenter > Rooms**. The table shows **Available Us** along with other details. You can filter the list by **Building** and **Tags** using the dropdown menus above the table. 

Click the **Show** link next to a room name to enter the [Room Layout View](#room-layout-view); this is a visual layout view of the room.
...
The right **Object List** panel displays the rooms that were included in the list page view – the previous screen you were on when you clicked the room name, with whatever filters were applied. If your user account does not have permission to view buildings, you will not see "@ - Building" next to the room name.
...
Use the zoom slider to increase or decrease the display size of the layout. Switch editing mode to on by clicking **Turn ON** in the sidebar. Only users with the [necessary permissions](/administration/role-based-access-control) can edit a room layout.
...
If you have [Power and Environmental Monitoring](/infrastructure-management/power-and-environmental-monitoring/) enabled, you will also see the following information:
...
- **Export selected items**: Open a window with the following export options:
  ...
  - **Selected Columns (CSV)**: Export data for a reduced set of important columns for buildings (`ID`, `Address`, `Image`, `Contact_Name`, `Contact_Phone`, and `Notes`) or rooms (`ID`, `Layout`, `Building`, `Available_Us`, and `Notes`) for the selected items in a CSV file.
