...
Navigate to **Infrastructure > DataCenter > Buildings** to view and manage buildings. Users with the [necessary permissions](/administration/role-based-access-control/index) can access the [Room Layout View](#room-layout-view) to see a visual representation of a room and its objects.

## Buildings List Page

You can view, edit, and **Create** new buildings from the Buildings list page, as well as perform bulk and individual **[Actions](#buildings-and-rooms-actions)** on selected buildings. 

To narrow down the list of buildings, filter it using the **Tags**, **Groups**, and **More Filters** dropdown menus above the grid.

You can add an image, like a floor plan or photo, to a building and view the image from the list page.
...
Click the **Impact Chart** button on the view page to open the chart in a new tab.
...
Rooms associated with the building are clickable links in the room layout view. Click **Edit** to add or remove rooms from a building.
  ...
  <ThemedImage
    ...
    sources={{
      light: useBaseUrl('/assets/images/buildings-and-rooms/buildings-with-room-links-light.png'),
      dark: useBaseUrl('/assets/images/buildings-and-rooms/buildings-with-room-links-dark.png'),
    }}
  />

## Rooms List Page

Find the Rooms list view under **Infrastructure > DataCenter > Rooms**. The table shows the **Available Us** along with other room details. You can filter the list by **Building** and **Tags** using the dropdown menus above the table. 

Click the **Show** link next to a room name to enter the [Room Layout View](#room-layout-view), which provides a visual layout of the room.

Perform bulk or individual operations on the selected rooms from the **[Actions](#buildings-and-rooms-actions)** dropdown menu.
...
Navigate between sections, including the following, using the left **Breakdown** panel:
...
<ThemedImage
  alt="Rooms List View"
  sources={{
    light: useBaseUrl('/assets/images/buildings-and-rooms/rooms-with-rack-links-light.png'),
    dark: useBaseUrl('/assets/images/buildings-and-rooms/rooms-with-rack-links-dark.png'),
  }}
/>
...
Use the zoom slider to increase or decrease the display size of the layout. Switch editing mode to on by clicking **Turn ON** in the sidebar. Only users with the [necessary permissions](/administration/role-based-access-control/index) can edit a room layout.
...
You can change the shape and size of the room layout. Add or remove rows and columns using the **+** (plus) and **-** (minus) buttons located on the left panel.
...
- Rotate objects by 90 or 180 degrees by hovering over the object and clicking the rotate buttons located on the popup panel. Edit mode must be enabled to see the rotate buttons.
...
If you have [Power and Environmental Monitoring](/infrastructure-management/power-and-environmental-monitoring/index) enabled, you will also see the following information:
...
## Buildings and Rooms Actions

The following operations are available from the **Actions** dropdown menu on the Buildings and Rooms list pages:

- **Delete with Detailed Confirmation**: View a list of all the objects that will be removed when you delete the selected buildings or rooms, like assets, devices, and racks.
- **Fast Background Delete**: Delete the building or room without first viewing the list of affected objects.
- **Export selected items**: Open a window with the following export options:
  - **Re-importable Export**: Export the selected buildings in a `.xlsx` spreadsheet to be re-imported into Device42.
  - **Buildings/Rooms Export (CSV)**: Export all data for the selected items in a CSV file.
  - **Selected Columns (CSV)**: Export only the data from a reduced set of important columns (`ID`, `Address`, `Image`, `Contact_Name`, `Contact_Phone`, and `Notes` for buildings and `ID`, `Layout`, `Building`, `Available_Us`, and `Notes` for rooms) for the selected items in a CSV file.

    <ThemedImage
      alt="Export options"
      sources={{
        light: useBaseUrl('/assets/images/buildings-and-rooms/buildings-export-options-light.png'),
        dark: useBaseUrl('/assets/images/buildings-and-rooms/buildings-export-options-dark.png'),
      }}
      style={{ width: '45%' }}
    />
