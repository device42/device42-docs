...
The sections below explain the main PDU fields, show how automatic port creation works, and outline how to map a PDU to a device when you want to track asset and lifecycle details. PDUs (like almost all Device42 CIs) can also be added and managed [via the API](https://api.device42.com/#pdu).
...
When you click **Save**, Device42 creates the new PDU and automatically generates ports based on the layout defined by the selected PDU model. For information about defining PDU models, see [Defining PDU Models](infrastructure-management/power-units/defining-pdu-models.mdx).
