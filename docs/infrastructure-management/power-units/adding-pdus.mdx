...
Note that PDUs (like almost all Device42 CI)s can also be added and manipulated [via the API](https://api.device42.com/#pdu).
...
To track details about a PDU (for examole, its serial number, asset number, IP address, purchase information, and life-cycle management), map the PDU to a device. Create a device for the purpose of PDU mapping as follows:
...
Navigating from a device to the PDU it's plugged into or from a PDU to the device page of the device that's plugged into it is simple. The PDU mapping links to the device details page (see the above image) and the device mapping page links back to the PDU details page (see the image below).
