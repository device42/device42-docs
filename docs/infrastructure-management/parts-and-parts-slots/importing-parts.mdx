...
Part models can be created using the **Parts - Create/Update Parts** import; the parts import can do both models and parts. The **Part Models - Create/Update Part Models** import can only create models.
...
Go to **Tools > Imports/Exports (xls)** and download current data for spare parts models as highlighted below.
...
Add a column for `partmodel_id` and populate the sample spreadsheet to add new parts to two of the existing part models you created. Add the serial numbers, which can be accomplished manually or via laser handheld scanner (see below).
...
You can use a DBPOWER scanner purchased on Amazon, which works great for this purpose. It comes with a USB receiver and can be programmed to automatically go to the next row in Excel, making it very easy and quick to capture serial numbers or other bar codes.
...
The new parts appear on separate line items, as these are unique line items for tracking serial numbers.
