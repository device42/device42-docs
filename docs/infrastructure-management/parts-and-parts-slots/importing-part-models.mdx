...
This document explains how to import part models using API Imports. For a detailed explanation of Part Models and Parts, see [Part Model Templates](infrastructure-management/parts-and-parts-slots/part-model-templates.mdx).
...
Hover over the upper-right corner of the column heading cells to see details about the field. These tooltips indicate whether fields are required, optional, or provide additional context.
...
Navigate to **Tools > Imports/Exports (xls)**, browse to you populated file, and select **Import** to begin the import process.
...
Device42 displays the status of the import. If there are any issues, the results will show "The following were Not added..." with specific error details. Review the results carefully, as any problems are usually clearly stated, such as missing required parameters.
...
Navigate to **Datacenter > Parts > Model List** to view all the part models that were imported in the previous step.
