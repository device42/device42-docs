...
The following sections describe the attributes and elements of `<task>` elements so you can understand what to adapt or emulate.
...
When you add the `<field>` elements, ensure that the mapped field types match. A mismatch between the Device42 source type and the Freshservice target type causes sync errors.
