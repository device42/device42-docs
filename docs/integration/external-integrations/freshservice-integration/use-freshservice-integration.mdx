...
- For help with sync and connection issues, see [Troubleshoot the Freshservice Integration](/integration/external-integrations/freshservice-integration/mapping-integration-troubleshooting).
:::
...
### Asset-Type Update Behavior

The **Update asset type for existing assets** checkbox controls whether asset types in Freshservice are updated during data synchronization (sync). Configure the following option [from the **Settings** tab](#find-the-settings-tab):

- For new installations, the asset-type checkbox **is selected by default.** Asset types in the mapping file will overwrite existing asset types in Freshservice. Uncheck this option if you want to preserve manual customizations in Freshservice after the sync runs.

- For existing installations, the asset-type checkbox is **not selected by default.** Asset types in Freshservice will not be overwritten by the sync. Existing installations are installations that didn't initially have the asset-type checkbox.

    ![Asset-type checkbox](/assets/images/freshservice-integration/sync-config-asset-type.png)
...
- Select **Run Partial Sync** (green button) to sync only the items that have been changed in Device42, either added or updated, since the last successful sync.
    ...
    ![Schedule synchronization](/assets/images/freshservice-integration/sync-config-schedule.png)
...
The following table maps Device42 data types to their corresponding categories in Freshservice:
