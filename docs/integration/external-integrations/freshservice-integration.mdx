...
We're actively working to expand and improve the Device42-Freshservice integration for specific assets. In 19.06, we added the following additional relationship types to map to the default Freshservice fields via the Cloud Connector:

- Kubernetes Cloud: Device42 can discover the relationships between the Kubernetes assets and make that data available in Freshservice, so you can see which assets are impacted when an incident or change-management request occurs.
- Google Cloud Platform (GCP): The relationship between Google Cloud virtual machines and their security groups has been added to the Device42-Freshservice integration.
- VMware VCenter
- Azure Cloud
- Amazon Web Services
