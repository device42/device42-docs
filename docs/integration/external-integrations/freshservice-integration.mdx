...
You'll see multiple `<task>` elements. There are tasks that copy data from Device42 to Freshservice, create relationships between Device42 assets, and delete assets or relationships from Freshservice if they don't exist in Device42. For example, there are tasks to bring in device data, bring in software data, and a task to create the device to software relationship from Device42 in Freshservice. 

You can modify and add to these tasks to customize the data mapping between Device42 and Freshservice.

Refer to [The `mapping.xml` File Tasks table below](#the-mappingxml-file-tasks) for descriptions of the default mapping tasks, types, and their version requirements. 

**Task Attributes**:

Turn off syncing for individual tasks by setting the `enable` attribute to `false`.
...
The following `<task>` element copies product information from Device42 to Freshservice:
