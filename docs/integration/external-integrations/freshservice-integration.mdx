...
## Examples of `mapping.xml` File Edits for Specific Use Cases

### Example 1: How can I adjust the sync to not import contracts from Device42 to Freshservice?

To disable the contract sync, set the `enable` attribute to `false` to the `<task>` elements with the descriptions:
- `Copy contract info from Device42 to Freshservice` (around line `2411).
- `Create association between asset and contract` (around line `2452).
- Optionally, you can disable the Software Licensing tasks, as it's also synced a contract type with the `Copy Software Licensing info from Device42 to Freshservice` task (around line `2493`).

Finding the tasks and disabling them looks like this:
...
2. Find the `Create association between asset and contract` task around line `2452` of `mapping.xml`:
    ...
    ```
3. If you also want to disable the Software Licensing sync, find the `Copy Software Licensing info from Device42 to Freshservice` task around line `2493`:
