...
- `type` : A [Freshservice classification](https://api.freshservice.com/) of the data. The valid types are: 
  - `product`
  - `asset`
  - `software`
...
To add a custom field to Freshservice, modify the `mapping.xml` file and reupload it. You'll need to modify a `<task>` that's most closely related to the custom field you want to create.
...
Depending on the custom field you want to add, you may have to modify the DOQL query in the `<resource>` element to fetch that data from Device42. Use the InsightsAI chat under **Analytics > InsightsAI** to help find the tables and fields you need to query, if needed.

If you can't find a task best fits your custom field, you can create a new `<task>` element in the `mapping.xml` file.

### How Do I Change an Existing Mapping to Pull From a Different Device42 Field?

Change the `resource` and `target` attributes of the `<field>` elements in the `<mapping>` element of the `<task>` that corresponds to the Device42 field you want to pull from
