...
Save the file and upload it back to Freshservice with the **Upload Custom Mapping** button [from the **Settings** tab of the **Configuration** page](#find-the-settings-tab).
...
3. A window will open, and mapped and unmapped data for the current device will be displayed.
...
Do a text search for tasks in the `mapping.xml` file that contain the word `contract` in their `description` attribute. You can also search for contract-related tasks in the [**The `mapping.xml` File Tasks**](#the-mappingxml-file-tasks) table.
...
4. Save the file and reupload it to Freshservice with the [**Upload Custom Mapping** button](#reupload-the-mappingxml-file). The "Contracts" mapping will no longer appear as a selection option in the **Data Mapping Chart** box
...
Do a text search for tasks in the `mapping.xml` file that contain the word `software` in their `description` attribute. You can also search for contract-related tasks in the [**The `mapping.xml` File Tasks**](#the-mappingxml-file-tasks) table. 
...
5. Run a [partial sync](#run-the-data-synchronization) to confirm that software items are no longer being synced, or wait until the scheduled sync runs.
    ...
    - Go to **Assets > Software** in the integration app. The page is populated before disabling software imports:

    ![Software before](/assets/images/freshservice-integration/software-freshservice-before-changes.png) 

    - After disabling software imports, the page will be empty:
...
In Device42, a Purchase Line Item is an individual entry associated with a larger Purchase (located under **Infrastructure > Purchases**). Contract Purchase Line Items sub-types include Lease, Warranty, Subscription, Service, and In-Service and (none) for unspecified types:
...
The Purchase Line Item **Lease** and **Warranty** contract types map to the Freshservice **Lease** and **Warranty** types. The other Purchase Line Item contract types (Service, In-Service, Subscription, none (unspecified)) are assigned to the Freshservice **Maintenance** type. 

![Synced Maintenance Contract Type in Freshservice](/assets/images/freshservice-integration/maintenance-type-contracts.png)

For example, this Purchase Line Item in Device42 with an unspecified contract type is synced to Freshservice as a **Maintenance** contract:
...
Software license contracts in Device42 are synced to Freshservice as **Software License** contracts when a Software Component has an associated Purchase Line Item or License Count that is not null (empty).

![Software-Licensing-Sync](/assets/images/Software-Licensing-Sync-250x110.png)

The DOQL query in the `mapping.xml` file for Software Licensing is:
...
## Business Applications Sync to Business Services in Freshservice

Using the default mapping, Business Applications in Device42 are synced to Freshservice as the Business Services asset type. The Business Application relationships are also synced.

- In Freshservice, navigate to **Assets > Inventory** (1) and select **Business Service** as the **Asset Type** (2) to view the synced Business Apps. Click on an item to view its details.
