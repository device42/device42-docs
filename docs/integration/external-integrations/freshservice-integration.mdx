...
You'll see multiple `<task>` elements. The tasks typically copy data from Device42 to Freshservice or create relationships between Device42 asset types. For example, from Device42, you can bring in device data, bring in software data, and then bring in the device to software relationship.

**Task Attributes**:

You can turn off syncing for individual tasks by setting the `enable=` attribute to `false`.

The `name` attribute specifies the name of the task and can be edited.

The `description` attribute provides a brief description of the task and can be edited.

The `d42_min_version` and `d42_max_version` attributes specify the minimum and maximum Device42 versions required to run the task. Tasks may have a maximum and/or minimum version requirement to run the or no version requirement. 

Refer to the [task summary table below](#the-mappingxml-file-task-types-and-descriptions) for the descriptions of all the top-level tasks and their version requirements.

**Task Elements**:

The `<api>` element contains the following elements related to the Device42 data:
- `<target>`: The default Device42 `asset-type` is equivalent to the Freshservice `type`. For example, the `Hardware` type in Device42 is `product` in Freshservice. In most cases, the `asset-type` value will be empty
- `<resource>`: Contains the DOQL query to fetch data from Device42.

The `<mapping>` element defines where the data is placed in Freshservice:
- `doql-suffix=`: The suffix to add to the DOQL query to fetch only the data that has changed since the last sync.
- `<field>`: The first field element is the data return from the query. The second field element is the mapping to the Freshservice item.
  - `target=`: The Freshservice field to map to. Visit the [Freshservice API documentation](https://api.freshservice.com/) to find the field names.
  - `escape=`: Some characters aren't supported in Freshservice fields and need to be escaped. Set to `true` by default, but if you encounter mismatches on the `name` field in particular, try setting the value to `false`.
...
Use this table to look up the default mapping tasks in the `mapping.xml` file that are one level deep in the `<tasks>` element.
...
### How Do I Troubleshoot Data Mapping Issues?
