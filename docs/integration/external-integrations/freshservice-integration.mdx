...
When creating a custom field in Freshservice to correspond to a Device42 field, you need to set the target field in the XML file to match Freshservice naming conventions. Verify the correct field names via the Freshservice API endpoint before running the sync. 

Field names in Freshservice are automatically generated according to the conventions:
- lowercase characters
- spaces are replaced with underscores 
- the asset type ID is appended to the end of the field name. This suffix is visible in the FS interface and not required in the XML file, but provides a way to verify the field name.
...
Verify the correct field name as follows:
1. Navigate to the edit page for the asset type under the Freshservice settings. See this [Freshservice Assets page](https://support.freshservice.com/support/solutions/articles/164410-different-types-of-assets-configuration-items-in-freshservice) if you need help locating the asset type's edit page. Copy the asset type ID in the URL of the edit page.
...
```
3. Search the returned JSON for the field label you are interested in to verify the correct field name. 
