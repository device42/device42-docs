...
- `enable`: You can turn off syncing for individual tasks by setting the `enable` attribute to `false`. For example:
    ...
    Note that if you turn off a task that has an `asset` or `asset_relationship` as its corresponding Freshservice type (for example, devices, business apps, and resources in Device42), you'll need to find and remove its corresponding query snippet in the `Delete` cleanup tasks. For an example, see [this answer in the FAQ section](#how-do-i-prevent-a-relationship-between-two-device42-asset-types-from-syncing-to-freshservice).
