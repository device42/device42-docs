...
## Examples of `mapping.xml` File Changes for Specific Use Cases
...
1. Around line `2411`, find the task with the description `Copy contract info from Device42 to Freshservice`. 
   
    ![Line 2411](/assets/images/freshservice-integration/example-vscode-line-2411.png)
...
2. Find the `Create association between asset and contract` task around line `2452` of `mapping.xml`:

    ![Line 2452](/assets/images/freshservice-integration/example-vscode-line-2452.png)
    ...
    ```
3. Optionally, if you want to disable Software Licensing from being synced, find the `Copy Software Licensing info from Device42 to Freshservice` task around line `2493`:

    ![Line 2476](/assets/images/freshservice-integration/example-vscode-line-2476.png)
...
1. Around line `2124` on `mapping.xml`, find the task with the description `Copy Softwares from Device42 to Freshservice using DOQL`. 
   
    ![Line 2124](/assets/images/freshservice-integration/example-vscode-line-2124.png)
...
2. There are two `Create Software Install from Software In Use` tasks; one with a `d42_min_version` attribute and one with a `d42_max_version`.  Set the `enable` attribute to `false` on both of them: 
    ...
    - The first task is around line `2140`:
    
    ![Line 2140](/assets/images/freshservice-integration/example-vscode-line-2140.png)
    ...
    - The second task is around line `2211`:

    ![Line 2211](/assets/images/freshservice-integration/example-vscode-line-2211.png)
