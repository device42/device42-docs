...
By default, Device42 does not update the `asset_state` values in Freshservice. The `mapping.xml` file includes a `skip-update="true"` attribute targeting the `asset_state` field to prevent overwriting values that may trigger Freshservice workflow automations.

If you want Device42 to update `asset_state` values (in Device42, the field is called `in_service`), set the `skip-update="true"` attribute to `skip-update="false"`. 

There are three `<field>` elements to update in version 2.0 of the default `mapping.xml` file:

1. On line 493 (for DOQL v2):
...
2. On line 1302 (also for DOQL v2):
