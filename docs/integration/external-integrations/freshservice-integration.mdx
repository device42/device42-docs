...
### How Do I Add a Custom Device42 Field to Freshdesk? (Overview)
...
### How Do I Sync a Device Custom Field to Freshservice? (Detailed Steps)

1. First create and add the new custom field in Device42. You can create a custom field for any CI type, but for this example we'll create a custom field called `Name` in the Devices section of Custom Fields in the Main Appliance (using the API is also an option, but this example uses the UI).
...
2. You will now see the custom field available for all device types. The example below shows the **Custom Fields** section for a Server device type named `centos-884163.device42.testing`. From this device details page, you can set a value for the custom field.

   ![Device custom field](/assets/images/freshservice-integration/server-custom-fields.png)

3. Next, add the new custom field to the `mapping.xml` file. Change the DOQL query in the `Copy Servers from Device42 to Freshservice using DOQL v2` task to map the new custom field to Freshservice. (There are normally three types of tasks with different `d42_min_version` parameters; update the DOQL for each of these in case you’re using older or different versions.)
    ...
    **Syntax notes**: You need to escape the double quotes (`”`) by using the `&quote;` HTML entity. The greater (`>`) and less than (`<`) symbols also need to be escaped. The equality operator works without special quotes. Please check existing DOQL for examples. 
   - A `LEFT JOIN` has been added since all custom fields in Device42 are stored in the `{CI name}_custom_fields` views. For example, the Devices CI is stored in `view_device_custom_fields_flat_v2`.

4. Now its time to map the custom field to Freshservice. Add a new `<field>` element in the `<mapping>` tag section of the XML file. You'll need to keep Freshservice open and follow the next step to get the exact Freshservice values to use in the new `<field>` element below.
   ...
   **Explanation**:
   ...
   - The `resource=` argument should be set to the name of the custom field in Freshservice. The custom field name in this example will be `cf_name`, following the Freshservice convention to use the display name (label) in snake case for the field name, occasionally with the ID of the asset type as a suffix. (See [Syncing to Custom Field Names in Freshservice](#syncing-to-custom-field-names-in-freshservice) for more details).
   - The `target-header=` argument is the name of the CI properties section of the heading name where your custom field will appear in Freshservice. In this case, `target-header=“Server”` refers to the “Server Properties” heading — there's no need to use "Properties", as it’s automatically added to every asset type.

5. In Freshservice, in the **Asset Types & Fields** section you will find the asset field which will be mapped from Device42. Since the device example is of type **Server**, it's under **Hardware > Computer > Server**.
   
    ![Custom field in Freshservice](/assets/images/freshservice-integration/server-freshservice.png)

6. Now lets edit the **Server** asset type by clicking on the **pencil icon** all the way on the right. 

    ![Edit the Server asset type](/assets/images/freshservice-integration/fs-pencil-icon.png)

    [The 'target' directive](#the-task-element-elements) should be set to the Device42 field to be mapped to Freshservice. 

    ![Label the custom field in the FS UI](/assets/images/freshservice-integration/fs-label-custom-field.png)

7. Finally, run the sync in Freshservice’s Device42 integration app. You should end up with the custom field we created and set in Device42:
