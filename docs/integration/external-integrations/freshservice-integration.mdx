...
- `name` : The task name can be edited and appears in the [data mapping table of the integration app](#data-mapping-between-device42-and-freshservice).
...
Typically, the first `<field>` represents the Device42 data returned from the query put in a field. The second element describes the equivalent field in Freshservice.
  ...
  - `source-type` : Not functionally critical. Used to provide the data type that's displayed on the [integration app's data mapping table](#data-mapping-between-device42-and-freshservice).
...
This section answers some common questions about data mapping between Device42 and Freshservice. We're actively working on improving the integration and this documentation. For technical questions please [contact support](support@device42.com) and use the **Page Feedback** button at the top of this page to leave feedback about the documentation. 
...
- Save the file and [upload it back to Freshservice](#reupload-the-mappingxml-file) with the **Upload Custom Mapping** button.
