...
In the `mapping.xml` file, do a text search for tasks containing the word `contract` in their `description` attribute. You can also search for contract-related tasks in [**The `mapping.xml` File Tasks**](#the-mappingxml-file-tasks) table.
...
In the `mapping.xml` file, do a text search for tasks that contain the word `software` in their `description` attribute. You can also search for contract-related tasks in the [**The `mapping.xml` File Tasks**](#the-mappingxml-file-tasks) table. 
...
### Example 3: How Do I Unlink the Freshservice Asset State From the Device42 In Service Field?    

1. In the `mapping.xml` file, add a 'mapping' to map the Device42 **In Service** field to an `in_service` custom field in Freshservice.

    - Include `view_device_v2.in_service as in_serv` in the `select view_device_v2` statement of the `<resource model="Devices" target="device42"` element:

    ![[Mapping to map to in_service field](/assets/images/freshservice-integration/in-service-field-mapping.png)](/assets/images/freshservice-integration/in-service-field-mapping.png)

    - Add the field mapping for `in_serv`:

    ```xml
    <field resource="in_serv" source-type="boolean" target="in_service" target-type="boolean" target-header="Hardware"/>
    ```

    ![[Field mapping for in_serv](/assets/images/freshservice-integration/in-service-field-mapping-2.png)](/assets/images/freshservice-integration/in-service-field-mapping-2.png)

2. In the `d42_sd_sync.py` script, note the following three changes we made to the `update_objects_from_server` method to prevent the Asset State field from changing during updates. Search for `# Custom` comments in the script.

    - **Customization 1:** Define the `asset_state_field_name` as `None`.
    ...
    - **Customization 2:** Check if the `map_info["@target"]` is `"asset_state"`, then assign the field name.
    ...
    - **Customization 3:** Prevent the Asset State field from being updated.
