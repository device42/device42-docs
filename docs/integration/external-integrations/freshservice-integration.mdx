...
### Example 3: How Do I Unlink the Freshservice Asset State from Device42's In Service Field?    

1. In the `mapping.xml` file, add a 'mapping to map to' an `in_service` custom field in Freshservice. 

    - Include `view_device_v2.in_service as in_serv` to the `select view_device_v2` statement in the `<resource model="Devices" target="device42"` element:
    ...
    - Add a field mapping for `in_serv`:
...
2. Note these three changes we made to the `d42_sd_sync.py` script in the `update_objects_from_server` method to prevent the Asset State field from changing during updates. Search for `# Custom` comments in the script.

    - Customization one: Define `asset_state_field_name` as `None`:
    ...
    - Customization two: Check if `map_info["@target"]` is `"asset_state"`, then assign the field name:
    ...
    - Customization three: Prevent the Asset State field from being updated:
