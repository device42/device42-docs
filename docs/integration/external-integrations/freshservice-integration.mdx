...
3. Select your **Cloud Connector** location or environment. It's set to **USA** by default.
    ...
    sources={{
        light: useBaseUrl('/assets/images/freshservice-integration/cloud-services-settings-light.png'),
        dark: useBaseUrl('/assets/images/freshservice-integration/cloud-services-settings-dark.png'),
    }}
    />
...
Configure the **Device42 for Freshservice** app to sync data from Device42 to Freshservice, and view and edit the mapped fields between Device42 and Freshservice.
...
- Customize the mapping between fields in Device42 and Freshservice by downloading, editing, and [reuploading](#reupload-the-mappingxml-file) the `mapping.xml` file. Click the **Download Default Mapping** button to download the `mapping.xml` file.
...
Open the `mapping.xml` file in a text or code editor to view and edit the data mapping. You'll see multiple `<task>` elements in the `mapping.xml` file. Version 1.0 of the file has around 6010 lines.
...
The `<api>` and `<mapping>` elements are nested in each task. Think of the `<api>` element as relating to Device42 data and the `<mapping>` element as relating to the Freshservice fields. Refer to the [Freshservice API documentation](https://api.freshservice.com/) for further details.
...
Each `<task>` element has the following `type`, `description`, and (`d42_min_version` and `d42_max_version`) version attributes.
...
### How Do I Disable a Relationship Between Two Asset Types?
...
This section provides specific examples of how to edit the `mapping.xml` file to change what is synced from Device42 to Freshservice.

### Example 1: How Do I Adjust the Sync To Not Import Contracts From Device42 to Freshservice?

To disable the contract sync, set the `enable` attribute to `false` to the `<task>` elements with the descriptions outlined in detail in the steps below:

1. Around line `2372`, find the task with the description `Copy contract info from Device42 to Freshservice`. 
...
2. Find the `Create association between asset and contract` task around line `2413` of `mapping.xml`:
    ...
    ```
3. Optionally, if you want to disable Software Licensing from being synced, find the `Copy Software Licensing info from Device42 to Freshservice` task around line `2437`:

    ![Line 2437](/assets/images/freshservice-integration/example-vscode-6.png)

    Change `enable="true"` to `false` as follows:
...
### Example 2: How Do I Adjust the Sync To Not Import Software From Device42 to Freshservice?

To disable software from syncing to Freshservice, set the `enable` attribute to `false` to the following`<task>` elements:

1. Around line `2087` on `mapping.xml`, find the task with the description `Copy Softwares from Device42 to Freshservice using DOQL`. 
...
2. There are two `Create Software Install from Software In Use` tasks; one with a `d42_min_version` attribute and one with a `d42_max_version`.  Set the `enable` attribute to `false` on both of them: 
    ...
    ```
3. Save the file and reupload it to Freshservice with the [**Upload Custom Mapping** button](#reupload-the-mappingxml-file).
