...
1. [In the `mapping.xml` file](#download-the-mappingxml-file), add a 'mapping' to map the Device42 `In Service` field to an `in_service` custom field in Freshservice.
...
2. Prevent the Asset State field from changing during updates in the `d42_sd_sync.py` script. [Download the updated file here](https://github.com/device42/freshservice/blob/64aaec2c940149237a1fff5c2c6c1dbd577f2e26/d42_sd_sync.py) or make the [following changes](https://github.com/device42/freshservice/pull/13/files) to the `update_objects_from_server` method in the script. Search for `# Custom` comments in the script to find the changes.
