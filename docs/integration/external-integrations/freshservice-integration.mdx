...
The mapping file includes `<field>` elements with `asset_state` as the target. The `skip-update` attribute is set to `"true"` to prevent overwriting values that may trigger Freshservice workflow automations. 
