...
:::note
You need to create a separate data source for each endpoint. If you want to add multiple endpoints, you must create a separate data source for each one.
:::
...
| Username         | A username is required to send credentials with requests, use any value, for example, `servicenow_data_source`                                     |
