...
- The Vault details listed under [Configuring HashiCorp Vault](#configuring-hashicorp-vault) below: an AppRole (Role ID and Secret ID) with read access to the required secret paths, the secret engine path, and the namespace if you use Multi-Tenancy
