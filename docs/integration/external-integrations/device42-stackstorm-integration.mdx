...
This page is for automation engineers who want to drive Device42 actions from StackStorm workflows. By the end, StackStorm can query and update Device42 in response to trigger events, using the Device42 integration plugin.
...
- A StackStorm instance. This guide runs StackStorm in a [Docker](https://docs.docker.com/get-docker/) container managed with [Docker Compose](https://docs.docker.com/compose/install/), which is suitable for testing but not production. See the [StackStorm installation options](https://docs.stackstorm.com/install/index.html) for production installs.
- A running Device42 instance, with Device42 credentials to configure the integration plugin
- Network connectivity between StackStorm and Device42
...
Configure the Device42 webhook to send data to StackStorm about updates to device lifecycle events. Use an [API key](https://docs.stackstorm.com/authentication.html#api-keys) rather than a temporary token for authentication.
...
The API key is only displayed once, so copy and save it in a password manager.
...
A more efficient way to retrieve all the IDs at once is to use a [DOQL query](https://docs.device42.com/reports/device42-doql/) against the [Device42 API](https://api.device42.com/#resource_Asset-Device_Life_Cycle).

Once you have all the lifecycle event IDs, create a JSON file to map each event to its corresponding object category. For each key-value pair, use the lifecycle event ID as the `name` and the object category name as the `value`, as in the example below. You can omit the `lc_` prefix from the ID number if you prefer.
...
For this example, the file is named `mydata.json`.

Add the JSON file to the StackStorm directory and [load the key-value pairs](https://docs.stackstorm.com/datastore.html#storing-and-retrieving-key-value-pairs-via-cli) into the StackStorm datastore (update the filename to match yours):
