...
[StackStorm](https://docs.stackstorm.com/overview.html#about) is well-suited to automate a range of IT management workflows. In response to a trigger event, you can integrate StackStorm and Device42 to update device details, set user permissions, send Device42 data to other systems, and more. 

## Set Up the Integration

Before creating a fully automated workflow, we'll install and configure StackStorm:

- Install StackStorm using Docker.
- Install the Device42 plugin.
- Configure the plugin with your Device42 credentials. 
- Test the integration by querying for Device42 data from StackStorm.

### Install StackStorm Using Docker

Running StackStorm in a [Docker](https://docs.docker.com/get-docker/) container, and using [Docker compose](https://docs.docker.com/compose/install/), is one of the quickest and easiest [StackStorm installation options](https://docs.stackstorm.com/install/index.html) available. This method is not designed to be used in production but will get StackStorm running quickly to test out automations.

Clone the plugin GitHub repository with `git clone https://github.com/stackstorm/st2-docker`, cd to the `st2-docker` folder, and use the `docker-compose up -d` command to start the docker containers. See the [StackStorm documentation](https://docs.stackstorm.com/install/docker.html) for more details.

### Log In to StackStorm

Start the convenient st2 command line from the docker container to log in to StackStorm:

1. Change into the directory with:
...
You can log in to StackStorm web UI using the default username `st2admin` and password `Ch@ngeMe`. Even if you're using the CLI, it's handy to have the StackStorm web UI open to quickly get feedback on communication attempts and see the available actions under the **Actions** tab. We'll use the `get_device_by_id` action later to test the communication between StackStorm and Device42.
...
Find the `device42.yaml` configuration file of the Device42 plugin and the following environment variables to it:
...
Use the StackStorm web UI to quickly enter your configuration information. Note that you should add a trailing slash to the `d42_server` endpoint despite the label saying not to.

![Configure via UI](/assets/images/stackstorm-integration/ss-ui-d42-config.png)

If you're using Docker compose and the st2 command line, configure the plugin in the `device42.yaml` file. There are two `device42.yaml` files. The plugin uses the `device42.yaml` file that is part of the Docker volume rather than the one within the container. By default you can find that config file here:
...
After saving the file, tell StackStorm to load the new values by running:
...
In Device42, we will create the following lifecycle stages/events along with new permission groups and users for each stage:
...
We're also going to create an [object category](https://docs.device42.com/administration/role-based-access-control/) for each lifecycle stage. When assigned to a device, the object category property grants role-based permissions to that device. 

In total, we'll add the following items to the Device42 main appliance:
...
Let's outline the automation we'll create with an example of a device moving from the purchasing to the mounting stage:
...
4. In Device32, the "mounting_cat" object category updates the device's user permissions to users in the "purchasing_group" permission group.
5. The "mounting" user will be able to see and edit the device, while these permissions are revoked for the "purchasing" user.

### Automation Setup Steps

Here's an outline of the steps we'll take to set up the automation:

**Creating the Device42 items**
   1. create the four permission groups
   2. create four users 
   3. link the permission groups to object categories
   4. create four lifecycle events
...
Test the automation by updating the asset lifecycle of a device to "mounting". Wait a few moments and refresh the device page. View the status of the webhook processing by going to **Tools > Webhooks > Pending**. You can view connection errors by viewing the endpoint under **Tools > Webhooks > Endpoints**

From the StackStorm side, you can look out for a POST request in the API logs:

```
2024-01-18 19:53:58,597 INFO [-] 7b2bc32f-6e47-45c7-ab12-d06258fcb582 - POST /executions with query={} (method='POST',path='/executions',remote_addr='172.18.0.14',query={},request_id='7b2bc32f-6e47-45c7-ab12-d06258fcb582')

2024-01-18 19:53:58,694 INFO [-] 7b2bc32f-6e47-45c7-ab12-d06258fcb582 - 201 2382 97.378ms (method='POST',path='/executions',remote_addr='172.18.0.14',status=201,runtime=97.378,content_length=2382,request_id='7b2bc32f-6e47-45c7-ab12-d06258fcb582')

```

The logs are located in the `/var/log/st2/st2api.log` file. If you're using docker, run the following command to see the logs in the terminal:
...
When the automation has been completed successfully, you'll see in Device42 that the **Object Category** value has changed to "mounting_cat" changing the **Group Permissions** to "mounting_group".
...
Now, if you log in with the "purchasing" user credentials and navigate to **Resources > All devices**, the device page list has no devices.
