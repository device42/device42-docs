...
### Use Query Language to Filter CI Parameters
...
- _Equals or less_ means to pass the validation, the custom field in the ticket should not contain any more CIs than the number specified in the configuration. Zero CIs are not allowed by this validation. Please note, if any other validation set up zero items, validation will not succeed.
...
There are four validation types that you can use to validate a specific parameter:

1. **Not Empty:** the parameter value should exist inside the configuration item. The additional value is not used.
2. **Exact Value:** the parameter value should exactly equal the value specified inside the row in the configuration.
3. **Contains the parameter:** the value should contain the value specified inside the row in the configuration. The value supports the filter operators and sequences, so you can use the same language you use for the project filters. For example, `(1|2),3` means that the configuration item parameter should contain `3` inside the text of the parameter, and additionally should contain `1` or `2`. For any parameter, all parameters will be checked and validation will pass if any parameter accepting the criteria found.
4. **Not Contains the parameter:** the value should not contain the value specified inside the row in the configuration. The possibilities for the value are the same as described in contains section.
