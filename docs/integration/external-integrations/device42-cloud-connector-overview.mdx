...
Device42 uses Cloudflare for the Cloud Connector. It is better to add the DNS of the Cloud Connector to your firewall than to add it to the Cloudflare IPs, as the DNS won't change. The table below lists the DNS addresses for each region. If you need specific IPv4 and IPv6 addresses, please refer to the Cloudflare [IP Ranges](https://www.cloudflare.com/en-in/ips/) documentation.
