...
- The **EnrichAI** option is **Enabled** by default. The data Normalization and Enrichment Service is a Device42 feature that uses reliable sources to populate additional device attributes, such as the vendor, OS, and key support dates. See the [Enriched Data page](auto-discovery/enrichai-data/index.mdx) for more details. If you do not want to use the Enriched Data service, you can disable it from this Cloud Services page.
