...
This page is for administrators who want to keep their Cherwell CMDB in sync with the infrastructure Device42 discovers. By the end, discovered assets and configuration items (CIs) from Device42 are synced into Cherwell, either once or on a schedule.
...
- A working Cherwell installation, with administrator access to Cherwell Service Management Administrator
- A running Device42 instance
- A Linux server with Python version 3.5 or later to run the integration script
- Network connectivity between the Linux server, Cherwell, and Device42
