...
SCCM discovery can be configured using WinRM, WMI (requires a WDS and read-only access to the SMS namespace), or direct database discovery.

## View, Run, and Add Jobs

Navigate to **Discovery > HyperVisors / \*nix / Windows** to view existing jobs, run a job, or create a new SCCM job by clicking **Create**.
...
Click on a job to view its details. The **Discovery Target** must be the database server and not other SCCM components. 

Modify the job by clicking **Edit**.
...
For discovery to return detailed information, read permissions are required for the following system views:
