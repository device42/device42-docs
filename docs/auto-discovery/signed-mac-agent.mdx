...
This will be an encrypted URL, and a public key with some other data needed for discovery. It uses symmetric AES-256 bit encryption.

The agent can be run as: `(sudo) <executable name> --config-file=<path>/d42_mac_signed_config`.
