...
We previously gathered all Oracle users and collected them as "Device42 Databases" by running the query:

```sql
SELECT * from SYS.ALL_USERS
```

We updated the query to retrieve Oracle users that have at least one associated database object:
