...
### Oracle Users and Database Discovery

Under the **Resource Properties** option, you can view the discovered **Oracle Database Name** and **Vendor Resource Type**. The **Vendor Resource Type** property distinguishes Oracle users that have associated DB objects from Oracle databases during discovery.

Discovered Oracle users with associated DB objects have a **Vendor Resource Type** of `Schema` and are listed in the **Database/Schema** table.

Oracle databases are assigned a **Vendor Resource Type** of `Database Instance`. 

![Oracle DB Resource Properties](/assets/images/database-discovery/oracle-vendor-resource-type.png)
