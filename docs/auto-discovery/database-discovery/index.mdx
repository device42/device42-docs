...
We previously gathered all Oracle users and collected them as "Device42 Databases", then we only retrieved Oracle users that have at least one associated database object, and from there we implemented a changes to ignore user schemas. As a result, some Oracle database connections may no longer show under specific user schemas. These 'orphaned' database connections are now listed under the main database instance instead of to a specific **Database/Schema** resource. While they’re not linked to individual schemas anymore, key details like the user login and schema name are still preserved in the database instance details.
...
sources={{
    light: useBaseUrl('/assets/images/database-discovery/db-discoveries-list-pg-light.png'),
    dark: useBaseUrl('/assets/images/database-discovery/db-discoveries-list-pg-dark.png'),
}}
/>
