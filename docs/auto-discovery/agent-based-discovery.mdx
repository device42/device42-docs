...
The Linux agent runs as `root` by default. However, it is possible to configure the agent to run with a limited, non-root account. Heres how you can create one:

1. **Create a Limited Account**:
   - Create a new Linux account (eg. `d42_limited`) without root access.
...
2. **Grant Sudo Permissions for Required Commands**:
   - Assign the account permission to run the commands required by the discovery agent. Edit the sudoers file using `visudo`:
     ```bash
     sudo visudo
     ```
...
- `arp`, `cat`, `chkconfig`, `crontab`, `curl`, `df`, `dmesg`, `dmidecode`, `docker`, `dpkg`, `fdisk`, `find`, `format`, `getconf`, `grep`, `hdparm`, `hostname`, `ifconfig`, `ip`, `iptables`, `lsof`, `lsblk`, `lspci`, `mount`, `netstat`, `ps`, `python3`, `rpm`, `service`, `systemctl`, `uname`, `wget`, and more as specified by your discovery requirements.

Ensure all these commands are executable by the limited account through `sudo` without requiring a password.
