...
We'll need to ensure all these commands are executable by the limited account through `sudo` without requiring a password.

Here's a shell script that can be used to check if the limited account has access to all the required commands:
