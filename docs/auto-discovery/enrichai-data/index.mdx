...
EnrichAI Data is a Device42 service for enhancing discovered data. Device42 supports the enrichment of Operating System (OS), vendor, and software data for discovered devices.

Device42 uses [Common Platform Enumeration (CPE)](https://en.wikipedia.org/wiki/Common_Platform_Enumeration) standards for consistency.

## Data Fields

The EnrichAI Data service populates the following vendor attributes, operating system attributes, software attributes, and support date fields according to the available data. The data is mined from validated, authoritative sources to ensure accuracy.
...
| Vendor Website          | The official vendor website URL                         |
| Legal Name              | The official legal name of the vendor entity            |
...
| Part/type         | CPE type: `a` (Application), `h` (Hardware), `o` (Operating System)         |
...
| Release             | The software release identifier (when applicable)            |
| Edition             | The software edition (such as Professional or Enterprise)    |
| Language            | The language variant (such as en-us for US English)          |
| Update              | Update identifier (such as `SP1` or `beta`)      |
| Target Software     | The target software platform (if applicable)               |
| Target Hardware     | The target hardware      |
...
Enriched data is available throughout Device42. You can view enriched software information on the [Software Components](/infrastructure-management/software/software-components.mdx) page and software lifecycle dates on the [Software EOL/EOS](/infrastructure-management/software/software-eol-eos.mdx) page. Vendor and OS enriched data is also available via API and Device42 Object Query Language (DOQL).
