...
During discovery, Device42 compares the email IDs of the discovered subscription user with the current End User list to check for a match. If a match is found, the subscription is linked to the existing **End User** as a **Software In Use** item.

If no match is found, a new End User with the software association will be created in Device42. 
