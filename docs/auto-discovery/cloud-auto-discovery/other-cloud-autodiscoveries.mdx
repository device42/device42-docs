...
A read-only Alibaba role can be provided as the discovery performs no changes to the cloud infrastructure during execution. Device42 needs an Alibaba account with the built-in permission set to `ReadOnlyAccess`.
