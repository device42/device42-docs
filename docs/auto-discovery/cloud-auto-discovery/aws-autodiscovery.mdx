...
All VMs in Device42 have their statuses normalized into one of three buckets: **Running**, **Stopped** or **Deleted**. This allows for a consistent understanding regardless of which hypervisor or cloud is used. 

Device42's statuses aren't mapped to the naming convention of the cloud provider. For example, [in AWS](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-instance-lifecycle.html), a VM states include **Running**, **Stopped**, and **Terminated**. In Device42, the **Terminated** state is mapped to **Deleted**.
