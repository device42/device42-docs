...
Device42 AWS discovery provides full visibility into your AWS environments, giving you insight into your total AWS consumption so you can optimize spending and services consumed. Device42 supports a wide range of AWS services including EC2, S3, RDS, Lambda, EKS, and more.

This page covers how to set up AWS discovery, the items discovered, IAM policy requirements, AWS Roles configuration, and environment credential setup using EC2 instance profiles.

## Discovery Items List Page

View the discovered AWS items by navigating to the list page under **Resources > Cloud Resources**. Use the search bar on the left or click one of the column dropdown menus to filter for specific AWS discovery item fields.
...
## Getting Started with AWS Discovery

To create an AWS discovery job:

1. Prepare your AWS account. You can use the policy example [shown below](#additional-endpoint-information).
2. Have your AWS Access Key and Secret Key ready, as Device42 uses them to perform discovery.
...
- Select **Discovery > Cloud** from the main menu and click **Create** at the top right of the Cloud Autodiscovery list page.
...
- Select **Use Environment Credentials** (see [Set Up Environment Credentials](#set-up-environment-credentials-using-ec2-instance-profiles) below), or add your Amazon **Access Key ID** and **Secret Key** for the account(s) to be discovered:

 Click the **magnifying glass icon** and **Add Password** in the upper-right corner. Enter your Access Key ID or Secret Key in the **Password**. Device42 uses encryption to store the keys.
...
**Note:** See [Add and Edit AWS Roles](#add-and-edit-aws-roles) below for instructions on creating the AWS Roles that Device42 displays for AWS cloud discovery jobs.
...
- Check **Extended Summary Discovery** to discover the full breadth of resources within your AWS environment. These resources are displayed in the **Cloud Resources** section with a limited dataset. 
...
Some discovery items require enabling the feature in the job configuration before they can be discovered.

| Cloud Service or Object Name | Where in Device42                      | Accessed API                             | Information Generated                                   | Permission(s) Required                                   |
| ------------------------- | --------------------------------- | ---------------------------------------- | ------------------------------------------------------ | ----------------------------------------------------- |
...
Device42 can discover the following S3 fields:
...
Device42 includes an editor to define or edit the AWS Roles displayed for Amazon AWS cloud discovery jobs. Follow the steps below to view and add AWS Roles.
...
Device42 adds the new AWS Role to the roles list. It will also appear in the **Available AWS Roles** list when you create or edit an Amazon AWS cloud discovery job.

The following steps are required to use the AWS switch (Assume) Role on the API calls to scan other AWS accounts.

**From the Main Account:**
    
  - Create a role within IAM using the **Another AWS Account** option.
        
  - Supply an account that uses **Account ID** and an account that uses the **Require External ID** option. There is no requirement for the MFA option at this time.
        
  - Use the [example IAM policy](#additional-endpoint-information) needed for discovery.
        
**From the sub-account (or separate account):**
...
### Set Up Dynamic Account Discovery Roles

You can discover all sub-accounts and add them to the discovery process using Dynamic Account Discovery.

Complete the following prerequisite steps before setting up the roles:
...
- Dynamic account discovery does not currently support discovering roles that use external ID values.
...
If you don't want to follow the steps above, you can either:
  ...
  - Attach the Device42 discovery policy to the user directly and select the **Discover main account** box on the job.

## Use AWS Roles to Discover Accounts Within Discovery Jobs

AWS cloud discovery jobs can use AWS roles to discover accounts. When the job includes an AWS role, the discovery job dynamically retrieves multiple accounts from AWS. A single role can discover multiple accounts, which enables you to set up discovery and specify the precise account to create, or leave the account empty to have the discovery job create cloud accounts as a result of the discovery.
...
Device42 statuses are not mapped to the naming convention of the cloud provider. For example, [in AWS](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-instance-lifecycle.html), VM states include **Running**, **Stopped**, and **Terminated**. In Device42, the **Terminated** state is mapped to **Deleted**.

## Set Up Environment Credentials Using EC2 Instance Profiles

You can perform AWS discovery without supplying long-term programmatic credentials by using Instance Profiles or IAM roles for Amazon EC2 instances.
...
Follow these steps to configure environment credentials for AWS discovery.
    ...
    On the IAM Policy creation screen, click **JSON** in the policy editor and paste one of the policies below based on your desired discovery configuration:
      ...
      - **Option 3:** Role Assumption Using Dynamic Account Discovery
          ...
          **Note:** This requires the associated Remote Collector or Main Appliance to be deployed within the organization's root (management) account.
          ...
          See [Set Up Dynamic Account Discovery Roles](#set-up-dynamic-account-discovery-roles) for more details on configuring Dynamic Account Discovery.
...
4. **Attach the role:**
    ...
    - From the **Modify IAM role** page, select the IAM Role created in Step 3 and click **Update IAM role**.

5. **Additional Member Account Configuration for Role Assumption Using Static or Dynamic Account Discovery**

    If you're configuring single account discovery, there are no remaining steps. If you've opted for role assumption using static or dynamic account discovery, continue with the steps below:
