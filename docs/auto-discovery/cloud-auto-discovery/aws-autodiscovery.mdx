...
The Device42 AWS autodiscovery feature ensures you have full visibility into your AWS environments. This visibility gives you insights of your total AWS consumption, allowing you to optimize spending and services consumed. New discoveries are added all the time based on the needs of our customers.
...
**From the sub- (or separate) account:** Assign a user to a role that has already been granted access as described in [Grant Access to the Role](https://docs.aws.amazon.com/IAM/latest/UserGuide/tutorial_cross-account-with-roles.html#tutorial_cross-account-with-roles-2).
...
5. **Additional Member Account Configuration for Role Assumption Using Static or Dynamic Account Discovery**
    ...
    - Create the discovery role in each member account to be discovered.
        ...
        Follow Step 3 again but this time select **Custom trust policy** instead of **AWS service**. Copy and paste the trust policy below and then at the add permissions screen, search for and select the discovery policy created in the previous step.
