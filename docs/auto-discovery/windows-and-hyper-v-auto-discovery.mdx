...
Device42 discovery can utilize multiple protocols to communicate with the target devices. Either WinRM or WMI can be used for Windows discovery. As of 18.10.00, WMI is the default protocol.
...
Our support team can provide best-effort assistance in trying to resolve issues. However, for both protocols, it is best to reach out to your network or system admin, to resolve connection issues.
...
- **LDAP Criteria:** Choose the LDAP Query to execute against AD, the resultant list which will then be targeted for Windows Autodiscovery. Select **Custom** to specify a custom LDAP filter or query.
...
Microsoft Local Admin Password Solution (LAPS) is a method of securing Active Directory (AD) member servers whereby the server's local admin password is randomly generated and stored as an attribute of that server's AD object in Active Directory. 
...
In environments with inconsistent naming conventions, select **Strip domain suffix** and **Hostname plus domain Name**. The options work together to strip the hostnames to short names and then append the found domains to it.

If your server hostnames include domain names, for example, `server1.domain.com`, and you **do not** strip the domain name suffix and choose the **Hostname plus Domain Name** option, the domain name will be appended again resulting in `server1.domain.com.domain.com`. 

Choose the **Hostname and add Hostname plus Domain Name as Alias** to add the discovered name as an alias, rather than replace an existing device's name. This option adds devices with the hostname as discovered as the device name, and the FQDN as the alias. 
...
For the **Domain Name**, we use the `Domain` value if the `DomainRole` value is one of the following: 
...
The Hyper-V server's hardware details and its name, UUID, and MAC addresses are pulled in from the VMs. In the **Server(s)** field, Be sure to include the Hyper-V servers as hostnames, IP addresses, or part of the CIDR/IP Range.

## Permission Requirements for WMI and Windows

Windows permissions requirements are broken down into two parts that are detailed in sections below:

- Minimum required permissions for discovery of **Windows hosts/guests information**.
- Minimum required permissions for the discovery of **services and application data** for [Application Dependency Mapping (ADM)] on Windows. 

Where applicable, WinRM-specific items are listed but permissions themselves must be enabled on the target machine.
...
1. Ensure at least **Enable Account**, **Remote Enable**, and **Read Security**, as well as **WMI permissions** are applied to the autodiscovery user account and to the following WMI namespaces and sub-namespaces:
...
:::note 
For Hyper-V discovery against Windows Server2k12 and newer: Should discovery fail due to a permissions error, you may need to add your Device42 discovery account to the built-in Hyper-V administrators group due to changes made with the way Microsoft verifies permissions on these newer operating systems.
:::
...
There are two options for configuring ADM permissions. The first option uses Local Administrative Permissions and the `IPC$` and `ADMIN$` shares. The second option lets users configure their own share.

**Local Administrator Method**

1. Application Dependency Mapping requires the local admin group on the target hosts and access to `IPC$` and `ADMIN$` shares.
...
When setting up the discovery job if the `IPC$` and `ADMIN$` shares are inaccessible you can now specify a network share to use. The share can be local to the device or a shared location on your network. You will need to give the scanning account read and write privileges to the new shared location. Note that this method also requires local admin permissions.
...
2. Change the WDS service user from `System` to one of the host users (like an admin account). You can try without this step, but users report failure without making this change as well.
