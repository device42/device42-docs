...
- **Exclusions:** Input any IP addresses or FQDNs to exclude from this discovery job.  
...
Newly created jobs will not run on the first day they are created, to prevent an unintentionally large amount of jobs from running initially. If you would like to run a job after its initial creation, select **Run Now** on the job details page.
