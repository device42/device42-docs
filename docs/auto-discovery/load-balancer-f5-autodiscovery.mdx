...
Create a job to collect load balancer configuration and connectivity data.
...
1. Start by scanning an F5 device via SNMP. Add a job under **Discovery > SNMP** and input the correct matching community string. Ensure you configure SNMP on your F5. See the [Load Balancers](load-balancers.mdx) page for more information about SNMP discovery.
...
2. Choose a Remote Collector (RC) if desired, specify the correct port, and select or clear the SSL option as needed.
