...
Citrix NetScaler load balancer API discovery is used to build TCP or UDP connections within application groups. NetScaler discovery will not add new devices and resources to the system. Adding new devices and resources is typically done via SNMP scans. 
