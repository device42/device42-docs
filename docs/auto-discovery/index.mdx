...
There are special situations when using an agent for discovery makes more sense. We offer optional autodiscovery agents for many platforms. See [Device42 Agent-Based discovery](agent-based-discovery.md) to learn more.
...
- See [Device42 Offline Agent-based discovery](agent-based-offline-discovery.md)
...
Use SNMP discovery to collect HP and IBM Blade System (or Blade Center) chassis and blade details. See [Blade Systems Autodiscovery](auto-discovery/blade-systems-auto-discovery.md) for more information. 
...
Prevent discovery from attempting to communicate with specific devices by adding them to the global exclusion list under **Tools > Global Settings**.
...
- [VMware and Virtual Machine discovery](virtual-machine-auto-discovery.md)
...
Visit [IPMI/Redfish Autodiscovery](ipmi-auto-discovery.md) for details.

## Midrange / Mainframe discovery (IBM AS/400 and z/OS)

Device42 supports agent-less mainframe and mid-range discovery of both the [IBM i / AS400 mid-range](ibm-i-as400.md) platform and the [IBM z/OS mainframe platform](z-os-ibm-mainframe.md).
...
There is also a ping sweep tool built into Device42. Find it in the UI under **Discovery > Ping Sweep**. Note that the UI-based option may be depreciated in the future and the standalone tool is much faster.
...
Windows scripts use PowerShell 2.0/iPython to get individual Windows host details and to get Windows machines from AD. Details are relayed to the Device42 Main Appliance via the API. The Linux, Solaris, and Mac scripts connect to remote servers using SSH to gather inventory data, sending that data back via the Device42 RESTful API.
...
See [Virtual Machine Autodiscovery](auto-discovery/virtual-machine-auto-discovery.md) for more information.
