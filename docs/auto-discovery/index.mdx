...
If you are new to Device42 discovery, see [Getting Started With Autodiscovery](/getstarted/getting-started-with-auto-discovery.mdx) for the recommended initial discovery sequence and setup guidance. For operational best practices on running and maintaining discovery jobs, see [Autodiscovery Best Practices](/auto-discovery/autodisc-best-practices.mdx).
...
There are special situations when using an agent for discovery makes more sense. Device42 offers optional discovery agents for many platforms. See [Device42 Agent-Based discovery](agent-based-discovery.mdx) to learn more.
