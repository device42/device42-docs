...
- Define the naming template pattern that will be used to generate unique names for the created discovery jobs. Include any credentials under **Username / Password(s)** that are needed to access the target servers.
...
- The **Device name format** settings deal with naming preferences for discovered devices and the scope of data that should be collected. 
...
- The next set of settings relate to the discovery of software and services with two cloud services options (**Discover ProviderID/Cloud ID** and **Use provider token**:).
  ...
  - **Store Application Components Config Files**: Visit the [Application Components](/docs/auto-discovery/application-components) page for more details on application components.
...
- Change the service level to the specified level when no VM is detected, assign the discovered devices to a VRF group, and associate specific customers to the discovered devices. 
...
- Elect for the generated discovery jobs to collect database information.
...
- Configure metadata settings by selecting an **Object Category for discovered devices** and entering **Tags** to apply to the devices. Tags are particularly useful to use with the [Application Dependency Mapping](/apps/enterprise-application-dependency-mapping/) feature of Device42.
