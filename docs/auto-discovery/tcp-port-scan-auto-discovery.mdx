...
- Specify the settings for the discovery of software and services, which include two cloud service options (**Discover ProviderID/Cloud ID** and **Use provider token**:).
  ...
  - **Store Application Components Config Files**: Visit the [Application Components](/auto-discovery/application-components) page for more information on application components.
