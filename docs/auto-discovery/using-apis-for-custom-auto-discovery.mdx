...
Device42 provides several [sample scripts](https://github.com/device42/Device42-AutoDiscovery-Scripts) you can use as a basis for custom discovery tools, including the following:
...
- Change the values for `USER` and `PASSWORD` to the credentials for an administrator user with login access to the Device42 appliance.
