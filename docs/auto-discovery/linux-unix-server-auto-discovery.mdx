...
Before configuring an SSH-based discovery job, please be sure you have your SSH port (standard TCP port 22, or a custom port) open between your Device42 Main Appliance or Remote Collector and the targeted Unix or Linux servers. 

Select **Discovery > Hypervisors / \*nix / Windows** from the main menu and the **+ Add Hypervisors/\*nix/win for Autodiscovery** button to add a discovery job to connect and gather host and VM details. 
...
Running `ssh-keygen` will ask you to set the file location of the key pair. You can use the default location - typically `$HOME/.ssh/id_rsa`. Note if a key already exists at this location, it will be overwritten unless you choose a different file location.
...
Instead of using `ssh-copy-id`, you can log into the target servers and manually set up the authentication: 
- Create the `.ssh` directory in the home directory you'll be using for discovery and create an **authorized_keys** file inside. 
...
- Copy the newly generated public key (copy the entire file) and paste it on a new line in the **authorized_key** file. It will look something like this: 
...
<ThemedImage
    ...
    sources={{
        ...
        dark: useBaseUrl('/assets/images/linux-unix-server-auto-discovery/device-name-format-options-dark'),
    }}
    style={{ width: '60%' }} 
/>
...
After saving a discovery job, you can run it immediately from the list, view, and edit pages using the **Run Now** button. You can also schedule the discovery job.
...
:::note
Newly created jobs will not run on the first day they are created to prevent an unintended large amount of jobs from running initially. If you would like to run a job after its initial creation, select the **Run Now** button next to the job after creation.
:::
...
The following is a table of commands we sometimes or always run as `sudo`. For the "Sometimes" commands, we'll try to run the command first without `sudo`, and if we receive a "permission denied" command rather than an "invalid command", "command not found", or similar we'll attempt to run it as `sudo`. This list will also say if the command is run on every Linux or UNIX flavor or only certain platforms:
