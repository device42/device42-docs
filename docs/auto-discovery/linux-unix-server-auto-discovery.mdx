...
If you experience unexplainable issues during discovery that you suspect is due to banner text interfering with parsing, enable the **Check for Banner** option for discovery to detect and remove banner messages. The **Check for Banner** option is also available via API.
...
Note that banner text interference can't be directly determined from Device42 and would require a power user with system access to the target machine to detect by switching users. Device42 support may advise you to select the banner option if they suspect banner text interference.  
