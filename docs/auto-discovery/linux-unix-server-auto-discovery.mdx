...
When discovery switches between *nix users, the triggered banner text messages may disrupt the database discovery and Application Component creation process and result in a discovery error or incomplete data collection. 

To ensure full data collection when switching users, check the **Check for Banner** option for discovery to detect and remove banner messages.
