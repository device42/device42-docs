...
To set up a Discovery Hub scan, name the scan and select the [Remote Collector](getstarted/deploy-device42/remote-collector-rc-installation.mdx) to use. You need to know the IPs, IP ranges, or CIDR blocks for your target environment.
...
## Configure a Global Blackout Window

Global blackout windows pause all automated discovery jobs during selected times. Use them to prevent discovery activity during maintenance, change freezes, or busy periods.

To create a blackout window:

1. Navigate to **Discovery > Discovery Hub**.
2. Click the **Global Discovery Settings** wrench icon.
3. Under **General**, find **Blackout Windows**.
4. Click **Add Blackout Window**.
5. Select the days, start time, and end time.
6. Click **Save**.

During the blackout window, Device42 suspends active and pending discovery jobs. When the window ends, the suspended jobs resume automatically.

<img
  alt="Create a global discovery blackout window"
  src={useBaseUrl('/assets/images/discovery-hub-onboarding/global-blackout-window.png')}
/>
