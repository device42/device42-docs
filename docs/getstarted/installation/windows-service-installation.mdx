...
```
#On Windows machine. Drop the agent under c:\\Program Files\\Device42 as d42winagent.exe  (make sure that agent is named that unless you are using d42config.json file to override that)
#for the command below notice the space after = sign below. It is critical.
...
#Also, note that sc create/start on Windows reads from %windir%/system32 by default. So if the user doesn't have C: or doesn't want to use C:\ProgramData\Device42, they can place d42config.json in the system32 directory
