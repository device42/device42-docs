...
3. Power on the new machine and log in to the console using the default credentials. Please change the credentials.
...
5. Point your browser to `https://FQDN (Fully Qualified Domain Name) or IP address`. Log in to the **web interface** using the default credentials:
...
What follows are steps to install Device42 with screenshots to illustrate the process.
...
Create a VM with a **minimum** of 2 vCPU, 8 GB RAM, and 1 network card, and configure the new VM to use the attached virtual disk file. See [Sizing Recommendations](sizing-recommendations.mdx) for more details.

:::note
Only HyperV Gen1 VMs are currently supported. In our experience, users have issues deploying Linux VMs as Generation 2 in HyperV. We recommend deploying Device42 only as a Gen1 VM.
:::
...
Change the default password for user `device42` from the console menu before proceeding.
