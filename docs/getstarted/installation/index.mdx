...
Note that while the above linked page includes an **Initial (First-Boot) Network Configuration** section for configuring the Remote Collector, we recommend you instead reference this [Remote Collector Configuration](/getstarted/installation/deployment-references/configuring-remote-collector/) page.
...
You can also install versions of the Windows and Mac discovery agent as a service. Click [here](/getstarted/installation/windows-service-installation/) for more information about the Windows Discovery Agent Service Wrapper.
...
Note the longer the duration, the more disk space will be used. For long term storage, you may leverage webhooks (click [here](/externalintegrations/webhooks-generic-integration) for more information about webhooks).
