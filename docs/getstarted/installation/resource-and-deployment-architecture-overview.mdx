...
The Main Appliance itself is managed using the [Appliance Manager (AM)](/administration/appliance-manager/index.mdx), which you can use to restart and stop the Device42 application, generate logs or SNMP walks for tech support, and more.
...
The Device42 deployment architecture is similar to a hub-and-spoke model, with each RC/WDS collecting data and syncing back to the centralized MA. We recommend you deploy RCs and WDS instances in pairs if you require Windows discovery.
