...
2. **Application Group Suggestions**: Review the suggested Application Groups. The list of suggestions is created by Calculation Rules that have their outcomes set to **Suggest**. Suggestions are also automatically generated by Application Components.
3. **My Application Groups**: Process the accepted Application Groups. Application Groups are generated based on Calculation Rules that have their outcomes set to **Auto-Create**. View charts of group dependencies and use the groups as components to build [Business Services](/apps/business-services). 
