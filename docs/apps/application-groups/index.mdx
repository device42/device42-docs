...
:::tip
Affinity Groups have been renamed **Application Groups**. AppFocus Filters are now called [**Application Group Calculation Rules**](calculation-rules.mdx).
:::

Application Groups are an opinionated view of the impactful applications and their dependencies in your environment. They are configurable via [Calculation Rules](calculation-rules.mdx) (and templates). Application Groups are based on the communication patterns observed in your environment.
...
Application Groups are calculated with [Calculation Rules](calculation-rules.mdx) based on [Logic Templates](calculation-rules.mdx#calculation-logic-templates), offering advanced capabilities and customizations beyond the previous reliance on pinning services as a starting point and DOQL queries. You can use a range of services, devices, and Application Components as starting points for your Application Groups. 

See the [Calculation Rules](calculation-rules.mdx) page to learn how to create and manage Calculation Rules and Logic Templates.

:::note
Application Groups cannot be created or updated through **Tools > Imports/Exports (xls)**. Use Application Group Calculation Rules instead.
:::
...
- Start by creating a new [**Calculation Rule**](calculation-rules.mdx)(1) to generate Application Group(s) and suggestions.
...
[Application Components](/apps/application-components/index.mdx) are displayed as stand-alone nodes, and are connected to any associated services and devices via dotted lines.  
