...
- Click on the nodes to see details about the resource. Click on relationship lines to see communication-related data (like the IP address and ports) between notes. If you don't see these data points, make sure you have the [**Store and Display Connection Metadata** option enabled in **Visualization** options](calculation-rules#visualization-options) of that Application Group's Calculation Rule.
