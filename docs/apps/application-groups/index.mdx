...
- **Application Group Suggestions** (2) are generated automatically from Application Components, but can also be generated via [Calculation Rules](calculation-rules#create-a-new-calculation-rule) when you select **Suggest** as the calculation **Outcome**. You can choose to accept or ignore the suggested groups. The suggested groups are added to the list under **Application Group Suggestions** (3) for processing.
