...
- **Application Group Suggestions** (2) are generated in two ways: automatically from Application Components and via [Calculation Rules](calculation-rules#create-a-new-calculation-rule) that have **Suggest** selected as the **Outcome**. You can choose to accept or ignore the suggested groups. The suggested groups are added to the list under **Application Group Suggestions** (3) for processing.

- Under the **My Application Groups** (3) tab, you can then bulk manage Application Groups using options from the **Actions** menu, view the [Impart Charts](#impact-charts) of your Application Groups, and search and filter the records. You can also pin Application Groups to keep important items at the top of the list.
...
Filter the devices list page to find any records with Application Group Calculations associated with that device within the last 30 days - either a client or a listener. 
...
- The Application Groups added will be visible on the Business Application canvas. The added devices are given colors based on their Application Groups. There are up to five different colors used to indicate which devices are from which Application Groups.
...
Any Application Component Categories associated with a device that is used to create an Application Group will also be associated with that Application Group. 
