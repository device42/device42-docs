...
- Use the controls at the left of the charts to highlight or search for dependencies and items of interest. 
...
- Click the relationship lines in the Global View to open the Local View panel displaying those relationships. On the Local View, click the dependency lines between services, processes, and devices to see connection stats (like the listener IP address, protocol, port, and average number of connections over time) . For lines to Application Components, the **Reason** and **What is affected** values are displayed if available.
