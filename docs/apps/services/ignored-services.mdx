...
Select one or more ignored services, and then select an **Action** to apply to them. You can use the **Action** menu to activate, deactivate, delete, or export selected ignored services.
...
To ignore dynamically named services, set **Ignored Level** to **Service Name** or **Display Name**, then enter an asterisk (`*`) as a wildcard in **Ignored Text**. For example, `PrintWorkflow*` matches names such as `PrintWorkflowUserSvc_56616f34` and `PrintWorkflow_56db1ef`. Entries without a wildcard continue to use exact matching.
