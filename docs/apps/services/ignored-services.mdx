...
To ignore dynamically named services by **Service Name** or **Display Name**, enter an asterisk (`*`) as a wildcard in **Ignored Text**. For example, `PrintWorkflow*` matches names such as `PrintWorkflowUserSvc_56616f34` and `PrintWorkflow_56db1ef`. Entries without a wildcard continue to use exact matching.
